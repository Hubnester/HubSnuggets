...
function init()
	...
	self.shipType = config.getParameter("shipType", "default")
end
...
function update(dt, fireMode, shiftHeld)
	...
	if storage.firing and animator.animationState("firing") == "off" then
		world.setProperty("hubsnuggetShipType", self.shipType)
...
function activate(fireMode, shiftHeld)
	if not storage.firing then
		if world.type() == "unknown" then
			...
			player.interact("ShowPopup", {message = "Only usable on your ship."})
		end
	end
end
